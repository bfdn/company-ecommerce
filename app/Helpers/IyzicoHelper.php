...
class IyzicoHelper
{
    ...
    public function createRequest($orderInfo, float $finalPrice)
    {
        ...
        $this->request->setConversationId($orderInfo->order_no); // herhangi bir sayı verilebilir, unique bir değer vermek mantıklı olabilir
        $this->request->setPrice($finalPrice);
        $this->request->setPaidPrice($finalPrice);
        $this->request->setCurrency(\Iyzipay\Model\Currency::TL);
        $this->request->setInstallment(1); // Taksit olayı
        $this->request->setBasketId("B67832"); // sepet id
    ...
    public function get($user, $shoppingCart, $creditCard)
    {
