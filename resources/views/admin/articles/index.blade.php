            ...
            @can('Yazı Ekle')
                <div class="row">
                    <div class="col-sm-12 d-flex justify-content-between">
                        <div></div>
                        <a href="{{ route('admin.article.create') }}" class="btn btn-success"><i class="bi bi-plus"></i>
                            Yazı Ekle</a>
                    </div>
                </div>
            @endcan
                            ...
                            <td>
                                @if (auth()->user()->can('Yazı Düzenle'))
                                    <x-elements.checkbox name="status" class="statusChange" :defaultValue="$item->status->value"
                                        dataUrl="{{ route('admin.article.statusChange', ['article' => $item->id]) }}"
                                        :formSwitch="true" />
                                @else
                            ...
                            <td>
                                ...
                                @can('Yazı Sil')
                                    <x-elements.button type="button" class="btn btn-danger btn-sm btnDelete"
                                        dataUrl="{{ route('admin.article.destroy', ['article' => $item->id]) }}">
