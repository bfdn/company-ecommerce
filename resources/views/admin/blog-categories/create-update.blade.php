...
@section('title')
    Blog Kategori {{ isset($item) ? 'Güncelleme' : 'Ekleme' }}
@endsection
@section('css')
...
@section('content')
    <x-elements.card>
        <x-slot:header>
            Blog Kategori {{ isset($item) ? 'Güncelleme' : 'Ekleme' }}
            <a href="{{ route('admin.blog-category.index') }}" class="btn btn-secondary float-end"><i
        ...
        <x-slot:content>
            ...
            <form
                action="{{ isset($item) ? route('admin.blog-category.update', ['blog_category' => $item->id]) : route('admin.blog-category.store') }}"
                id="Form">
                @csrf
